    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/api/users/login", "/api/users/register", "/api/users/refresh", "/api/users/duplicate/**", "/swagger-ui/**", "/v3/api-docs/**",
                                "/api/oauth/**", "/api/ai/generate/**", "/api/users/email/**", "/api/users/password", "/api/interests/**", "/api/auth/reissue").permitAll()  // 허용 경로 설정
