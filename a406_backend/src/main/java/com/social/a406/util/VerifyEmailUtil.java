...
public class VerifyEmailUtil {
    ...
    public String sendEmail(String to, String verifyCode) {
        try {
            ...
            helper.setTo(to);
            helper.setSubject("[Fillit] Email Verification Code");
            helper.setFrom(serviceEmailAddress);

            String htmlContent =
                    "<div style='font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;'>"
                            + "<div style='text-align: center;'>"
                            + "<img src='https://fillit-db.s3.us-east-2.amazonaws.com/etc/%EB%A1%9C%EA%B3%A0_(1).png' alt='FiLLit Logo' style='max-width: 150px; height: auto; margin-bottom: 20px;'/>"
                            + "</div>"
                            ...
                            + "<div style='padding: 15px; background-color: #f3f3f3; text-align: center; border-radius: 5px; font-size: 24px; font-weight: bold; color: #2c3e50;'>"
                            + verifyCode + "</div>"
